...
pipeline {
    ...
    parameters {
        choice(
            name: 'BUILD_TYPE',
            choices: ['Both', 'Frontend Only', 'Backend Only'],
    ...
    stages {
        stage('Checkout All Repositories') {
            ...
            steps {
                script {
                    // Checkout frontend only if building Frontend or Both
                    if (env.BUILD_TYPE == 'Both' || env.BUILD_TYPE == 'Frontend Only') {
                        dir('frontend') {
                            checkout scm
                    ...
                    // Checkout backend only if building Backend or Both
                    if (env.BUILD_TYPE == 'Both' || env.BUILD_TYPE == 'Backend Only') {
        ...
        stage('Load Backend Environment Variables') {
            when {
                expression { env.BUILD_TYPE == 'Both' || env.BUILD_TYPE == 'Backend Only' }
            }
            steps {
            ...
            steps {
                script {
                    ...
                    if (parallelBuilds.size() > 0) {
                        echo "Building services in parallel: ${parallelBuilds.keySet()}"
                        parallel parallelBuilds
                    } else {
                        echo "No services selected for building!"
                    }
                }
            }
        }
        ...
        stage('Docker Build All Services') {
            when {
                expression { env.BUILD_TYPE == 'Both' || env.BUILD_TYPE == 'Frontend Only' }
            }
            steps {
                script {
        ...
        stage('Deployment Ready') {
            steps {
                script {
                    def builtServices = []
                    if (env.BUILD_TYPE == 'Both' || env.BUILD_TYPE == 'Frontend Only') {
                        builtServices.addAll(["shared-lib", "root-config", "root-container", "tb-cybersio-ui", "tb-thirdParty-ui", "tb-rbac"])
                    }
                    if (env.BUILD_TYPE == 'Both' || env.BUILD_TYPE == 'Backend Only') {
                        builtServices.addAll(["dist-api-gateway", "tb-rbac-backend"])
                    }
                    ...
                    echo """
                        ...
                        Services Built:
                        ${builtServices.collect { "✓ ${it}:${params.VERSION}" }.join("\n")}
    ...
    post {
        success {
            echo """
                🎉 SUCCESS: ${env.BUILD_TYPE} build completed for version ${params.VERSION}
                📍 All services built on agent-127 (192.168.0.127)
            """
        }
        failure {
            echo """
                ❌ FAILURE: ${env.BUILD_TYPE} build failed for version ${params.VERSION}
