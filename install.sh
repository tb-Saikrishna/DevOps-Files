...
replace_compose_values() {
  ...
  # HOST IP / DOMAIN
  # ==========================================
  sed -E -i "s|^([[:space:]]*-[[:space:]]HOST_IP=).*|\\1${HOSTNAMES[tbsiem]}|g" "$DOCKER_COMPOSE_FILE"

  # ==========================================
  # TBSIEM BASE URL
  # ==========================================
  sed -E -i "s|^([[:space:]]*TBSIEM_BASE_URL:).*|\\1 https://${HOSTNAMES[tbsiem]}:8003|g" "$DOCKER_COMPOSE_FILE"

  # ==========================================
  # OpenSearch URL
  # ==========================================
  sed -E -i "s|^([[:space:]]*SERVER_IP:).*|\\1 '$SERVER_IP_JSON'|g" "$DOCKER_COMPOSE_FILE"

  # ==========================================
  # Wazuh Manager URL
  # ==========================================
  sed -E -i "s|^([[:space:]]*TBSIEM_CLIENT_API_URLS:).*|\\1 https://${WAZUH_MANAGER_HOST}:55000|g" "$DOCKER_COMPOSE_FILE"

  # ==========================================
  # API Gateway URL
  # ==========================================
  sed -E -i "s|^([[:space:]]*API_GATEWAY_BASE_URL:).*|\\1 https://${CYBERSIO_HOST}:9007|g" "$DOCKER_COMPOSE_FILE"

  # ==========================================
  # SOAR URL
  # ==========================================
  sed -E -i "s|^([[:space:]]*SOAR_BASE_URL:).*|\\1 https://${SOAR_HOST}:5001|g" "$DOCKER_COMPOSE_FILE"
}
